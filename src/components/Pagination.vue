...
*****************************************************************************/
<template>
    <div class="pagination-container" :style="[{background: currentData.paginationBackground && showStyle}]"
         :class="{'pagination-position':!showStyle}">
        <!-- 批量操作按钮 -->
        <div class="batch-handle" v-if="currentData.showBatchButtons">
            <span :class="{'show-checked':true,'active': checked}" @click="selectEvt"></span>
            <a href="javascript:void(0)" v-for="button in currentData.batchButtons" :now-id="button.id"
               ref="batchButton">{{button.text}}</a>
        </div>
        <!-- 分页内容 -->
        <div class="pagination-content">
            ...
            <ul class="pagination" :style="[{float: currentData.paginationPosition}]">
                <li :class="{'disabled': current == 1}"
                    :style="[{'border-width': currentData.hasPageGroupborder ? '1px' : 0}]">
                ...
                <li v-for="p in grouplist"
                    :style="[{background: current == p.val ? currentData.pageGroupHoverBackground : 'none'},{'border-color': current == p.val ? currentData.pageGroupHoverBackground : currentData.pageGroupborderColor},{'border-width': currentData.hasPageGroupborder ? '1px' : 0}]">
                    <a href="javascript:;"
                       :style="[{color: current == p.val ? currentData.pageGroupHoverColor : '#333'}]"
            ...
            <!-- 分页展示信息 -->
            <div class="pagination-msg" v-if="showTotal">
				<span>共有{{currentData.total}}条，每页显示
					<select v-model="currentData.rows">
						<option :selected="item.value == currentData.rows" :value="item.value"
                                v-for="item in currentData.rowsData">{{item.text}}</option>
					</select>条
				</span>
            </div>
        </div>
    ...
    export default{
        ...
        watch: {
            'currentData.rows': function (val) {
                let that = this;
                that.$nextTick(()=>{
                    that.currentData.rows = val;
                    /* 告诉父窗口改变rows数量 */
                    this.$emit('paginationChange',  that.currentData);
                });
            },
            'paginationData._reloadFlag': function (val) {
                let me = this;
            ...
            "paginationData.total":function(val){
                this.currentData.total = val;
            },
        },
        /**
        ...
        methods: {
            selectEvt: function () {
                ...
                that.$emit('paginationCheck', that.checked);
            },
